...
export function GhostLogic(ghost: THREE.Group, type: GhostNames) {
	...
	const moveGhostToCenter = new Tween(ghost.position)
		.to({ x: 0, y: 0.35 }, 2000)
		.delay(timeInterval!);
	const moveGhostToTheBoard = new Tween(ghost.position)
		.to({ x: 0, y: 1.25 }, 2000)
		.onComplete(() => (ghostFree = true));
	moveGhostToCenter.chain(moveGhostToTheBoard).start();

	function updateGhost() {
		if (ghostFree) {
			const possibleMapColision = mapColisionChecker(
				new Vector2(ghost.position.x, ghost.position.y),
				pacmanMap
			);
			if (possibleMapColision) {
				const chooseXorY = Math.floor(Math.random() * 2);
				const possibleDirections = <MapPointDirections>(
					possibleMapColision[1]
				);
				if (chooseXorY === 0) {
					const possibleValues = Math.floor(
						possibleDirections.x.length * Math.random()
					);
					ghostDirection = new THREE.Vector2(
						possibleDirections.x[possibleValues],
						0
					);
				} else if (chooseXorY === 1) {
					const possibleValues = Math.floor(
						possibleDirections.y.length * Math.random()
					);
					ghostDirection = new THREE.Vector2(
						0,
						possibleDirections.y[possibleValues]
					);
				}
			}
